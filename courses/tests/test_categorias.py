...
from courses.tests.helpers import create_category
...
class CategoryPermissionTests(TestCase):
    ...
    def test_regular_user_cannot_create_category(self):
        resp = auth_client(self.student).post("/api/categories/", {"name": "Test", "slug": "test"})
        self.assertEqual(resp.status_code, status.HTTP_403_FORBIDDEN)

    def test_admin_can_create_category(self):
        resp = auth_client(self.admin).post(
            "/api/categories/", {"name": "Frontend", "slug": "frontend"}
        )
